...
-- Using IF NOT EXISTS and OR REPLACE to handle existing objects

-- 1. Create audit log table for security events (drop and recreate to ensure clean state)
...
-- 2. Create session anomaly tracking (drop and recreate to ensure clean state)
DROP TABLE IF EXISTS public.session_anomalies CASCADE;
...
-- Create policy for session anomalies
...
-- 8. Apply admin activity logging to sensitive tables (drop existing triggers first)
DROP TRIGGER IF EXISTS audit_demo_access_codes_changes ON public.demo_access_codes;
DROP TRIGGER IF EXISTS audit_demo_requests_changes ON public.demo_requests;
...
-- 10. Create a function to get security dashboard data
